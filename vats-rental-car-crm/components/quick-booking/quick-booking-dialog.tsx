...
import { Plus, Car, User, Calendar, AlertTriangle, CheckCircle, Clock, IndianRupee, Camera } from "lucide-react"
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      ...
      const booking = {
        ...
        representativeName: bookingData.representativeName,
        representativeSelfie: bookingData.representativeSelfie,
      }
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {step === 4 && (
          <div className="space-y-4">
            ...
            <div className="border rounded-lg p-4 space-y-4">
              <h4 className="font-medium">Representative Information</h4>
              <div>
                <Label>Representative Name</Label>
                <Input
                  value={bookingData.representativeName}
                  onChange={(e) => setBookingData((prev) => ({ ...prev, representativeName: e.target.value }))}
                  placeholder="Enter representative's name"
                />
              </div>
              <div>
                <Label>Representative Selfie</Label>
                <SelfieCapture
                  onPhotoCapture={(dataUrl) => {
                    setBookingData((prev) => ({ ...prev, representativeSelfie: dataUrl }))
                  }}
                  triggerButton={
                    <Button variant="outline" type="button" className="w-full">
                      <Camera className="h-4 w-4 mr-2" />
                      {bookingData.representativeSelfie ? "Retake Selfie" : "Capture Selfie"}
                    </Button>
                  }
                />
                {bookingData.representativeSelfie && (
        ...
        {step === 5 && (
          <div className="space-y-4">
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              ...
              {bookingData.representativeName && (
                <div className="border-t pt-3">
                  <span className="text-gray-600 text-sm">Representative:</span>
                  <div className="flex items-center gap-3 mt-1">
                    {bookingData.representativeSelfie && (
                      <img
                        src={bookingData.representativeSelfie}
                        alt="Representative"
                        className="w-10 h-10 rounded-full object-cover border-2 border-gray-300"
                      />
                    )}
                    <span className="text-sm font-medium">{bookingData.representativeName}</span>
                  </div>
                </div>
              )}
            </div>
...
// components/quick-booking/quick-booking-dialog.tsx

import { useState } from "react";
import { Input, Label, Button } from "@/components/ui"; // Ensure the components are imported correctly
import SelfieCapture from "@/components/camera/selfie-capture";
      ...
      <div className="border rounded-lg p-4 space-y-4">
        ...
        <div>
          ...
          {bookingData.representativeSelfie && (
            <div className="mt-2 flex items-center gap-2 text-sm text-green-600">
              <span>Selfie captured</span>
              <img src={bookingData.representativeSelfie} alt="Representative" className="w-10 h-10 rounded-full object-cover" />
            </div>
          )}
        </div>
      </div>

      {/* Continue the existing form rendering */}
    </>
  );
}