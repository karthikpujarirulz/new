...
export class EnhancedPDFService {
  ...
  private addDocumentRequirements(doc: jsPDF, customer: Customer, yPos: number): number {
    ...
    if (customer.aadharUrl || customer.dlUrl || customer.photoUrl) {
      doc.setFontSize(12)
      doc.setFont("helvetica", "bold")
      doc.setTextColor(44, 62, 80)
      ...
      if (customer.aadharUrl) {
        try {
          doc.addImage(customer.aadharUrl, 'JPEG', xPos, yPos, imageWidth, imageHeight)
          doc.setFontSize(8)
          doc.setFont("helvetica", "normal")
          doc.setTextColor(60, 60, 60)
          doc.text("Aadhar Card", xPos, yPos + imageHeight + 5)
          xPos += imageWidth + 10
        } catch (error) {
          console.log("Could not add Aadhar image to PDF:", error)
        }
      }

      if (customer.dlUrl) {
        try {
          doc.addImage(customer.dlUrl, 'JPEG', xPos, yPos, imageWidth, imageHeight)
          doc.setFontSize(8)
          doc.setFont("helvetica", "normal")
          doc.setTextColor(60, 60, 60)
          doc.text("Driving License", xPos, yPos + imageHeight + 5)
          xPos += imageWidth + 10
        } catch (error) {
          console.log("Could not add DL image to PDF:", error)
        }
      }

      if (customer.photoUrl) {
        try {
          doc.addImage(customer.photoUrl, 'JPEG', xPos, yPos, imageWidth, imageHeight)
          doc.setFontSize(8)
          doc.setFont("helvetica", "normal")
          doc.setTextColor(60, 60, 60)
          doc.text("Customer Photo", xPos, yPos + imageHeight + 5)
        } catch (error) {
          console.log("Could not add customer photo to PDF:", error)
        }
      }
